...
class Partner(models.Model):
    ...
    sharevault_ids = fields.One2many(
        'sharevault.sharevault', 'partner_id', 'ShareVaults')
    sharevault_ids_count = fields.Integer(
        'ShareVault count', compute='get_sharevault_count')
    auditlog_ids_count = fields.Integer(
    ...
    european_union = fields.Boolean(
        'Are you a citizen or resident of the European Union (EU)?')
    ...
    account_status_id = fields.Many2one(
        'res.partner.account_status', 'Account Status')
    data_source_id = fields.Many2one('res.partner.data_source', 'Data Source')
    data_update_request_id = fields.Many2one(
        'res.partner.data_update_request', 'Data Update Request')
    lead_type_id = fields.Many2one('res.partner.lead_type', 'Lead Type')
    lifecycle_stage_id = fields.Many2one(
        'res.partner.lifecycle_stage', 'Lifecycle stage')
    organization_type_id = fields.Many2one(
        'res.partner.organization_type', 'Organization Type')
    ...
    contact_type_id = fields.Many2one(
        'res.partner.contact_type', 'Contact Type')
    matter_id = fields.Many2one(
        'res.partner.matter', 'Subject matter most interested in?')
    tecnology_id = fields.Many2one(
        'res.partner.tecnology',
        'Technology solution used to share documents with 3rd parties?')
    confidential_id = fields.Many2one(
        'res.partner.confidential',
        'When will be sharing confidential information with a third party?')
    ...
    sharevault_activated_user = fields.Selection(
        [('null', 'null')], 'ShareVault Activated User')
    sharevault_admin = fields.Selection([('null', 'null')], 'ShareVault Admin')
    sharevault_domain = fields.Selection(
        [('null', 'null')], 'ShareVault Domain')
    sharevault_email_subscription = fields.Selection(
        [('null', 'null')], 'ShareVault Email Subscription')
    sharevault_publisher = fields.Selection(
        [('null', 'null')], 'ShareVault Publisher')
    ...
    def _get_name(self):
        """ Utility method to allow name_get to be 
        overrided without re-browse the partner """
        partner = self
